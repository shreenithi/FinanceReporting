...
namespace FinReport.tools
{
    ...
    public class DatabaseAccessForReporting
    {
        ...
        public List<SearchModel> Search(string name, string zipCode)
        {
            ...
            {
                command.CommandText = "SELECT f.customerid,f.firstname,f.lastname,f.customerstatus,f.zipcode,f.email,f.contactnumber,d.due_amount,((SELECT SYSDATE FROM DUAL) - d.days_elapsed) AS payment_date FROM fincustomerdata f, dlqtable d  WHERE f.customerid = d.account_number AND f.zipCode = :zip";
        ...
        {
            EstablishConnection();
            OracleCommand command = new OracleCommand("SELECT f.customerid,f.firstname,f.lastname,f.customerstatus,f.zipcode,f.email,f.contactnumber,d.due_amount,((SELECT SYSDATE FROM DUAL) - d.days_elapsed) AS payment_date FROM  fincustomerdata f, dlqtable d WHERE  d.account_number= f.customerid", conn);
            OracleDataReader reader = command.ExecuteReader();
            List<SearchModel> list = new List<SearchModel>();
            while (reader.Read())
            {
        ...
        {
            EstablishConnection();
            OracleCommand command = new OracleCommand("SELECT f.customerid,f.firstname,f.lastname,f.customerstatus,f.zipcode,f.email,f.contactnumber,d.due_amount,((SELECT SYSDATE FROM DUAL) - d.days_elapsed) AS payment_date FROM fincustomerdata f, dlqtable d WHERE  d.account_number= f.customerid AND d.flag=0", conn);
            OracleDataReader reader = command.ExecuteReader();
            List<SearchModel> list = new List<SearchModel>();
            while (reader.Read())
            {
